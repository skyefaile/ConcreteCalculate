    ...
    public partial class Form1 : Form
    {
        ...
        private void buttonCalculate_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                // Validate length
                if (!double.TryParse(textBoxLength.Text, out length) || length <= 0 || length > 100)
                {
                    MessageBox.Show("Length must be a number from 0 and 100.");
                    textBoxLength.Focus();
                    return;
                }

                // Validate width
                if (!double.TryParse(textBoxWidth.Text, out width) || width <= 0 || width > 100)
                {
                    MessageBox.Show("Width must be a number from 0 and 100.");
                    textBoxWidth.Focus();
                    return;
                }
